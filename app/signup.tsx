...
export default function SignupScreen() {
  ...
  const [barbellWeightError, setBarbellWeightError] = useState('');

  // Calculate maximum allowed barbell weight (33% of person weight)
  const getMaxBarbellWeight = () => {
    const weight = parseFloat(personWeight);
    return weight ? (weight * 0.33).toFixed(1) : 0;
  };

  // Validate barbell weight whenever person weight or barbell weight changes
  useEffect(() => {
    if (personWeight && barbellWeight) {
      const personWeightNum = parseFloat(personWeight);
      const barbellWeightNum = parseFloat(barbellWeight);
      const maxAllowed = personWeightNum * 0.33;
      
      if (barbellWeightNum > maxAllowed) {
        setBarbellWeightError(`Barbell weight cannot exceed ${maxAllowed.toFixed(1)}kg (33% of your body weight)`);
      } else {
        setBarbellWeightError('');
      }
    } else {
      setBarbellWeightError('');
    }
  }, [personWeight, barbellWeight]);

  // Handle person weight change and auto-adjust barbell weight if needed
  const handlePersonWeightChange = (value: string) => {
    setPersonWeight(value);
    
    // If barbell weight is already entered and exceeds the new limit, clear it
    if (barbellWeight && value) {
      const personWeightNum = parseFloat(value);
      const barbellWeightNum = parseFloat(barbellWeight);
      const maxAllowed = personWeightNum * 0.33;
  ...
  // Handle barbell weight change with validation
  const handleBarbellWeightChange = (value: string) => {
  ...
  const handleSignup = async () => {
    ...
    if (personWeight && barbellWeight) {
      ...
      if (barbellWeightNum > maxAllowed) {
        showError(`Barbell weight cannot exceed ${maxAllowed.toFixed(1)}kg (33% of your body weight)`);
        return;
      }
    }
        ...
        <View style={styles.content}>
          ...
          <View style={styles.weightRow}>
            <View style={[styles.inputContainer, styles.halfInput]}>
              ...
              <TextInput
                ...
                value={personWeight}
                onChangeText={handlePersonWeightChange}
              />
            </View>
            ...
            <View style={[styles.inputContainer, styles.halfInput]}>
              <Text style={[styles.label, { color: theme.text }]}>
                Barbell Weight (kg)
                {personWeight && (
                  <Text style={[styles.maxWeightText, { color: theme.textSecondary }]}>
                    {' '}(max: {getMaxBarbellWeight()}kg)
                  </Text>
                )}
              </Text>
              <TextInput
                style={[
                  ...
                  { 
                    backgroundColor: theme.surface, 
                    color: theme.text,
                    borderColor: barbellWeightError ? '#ff4444' : 'transparent',
                    borderWidth: barbellWeightError ? 1 : 0,
                  }
                ]}
                ...
                keyboardType="numeric"
                value={barbellWeight}
                onChangeText={handleBarbellWeightChange}
              />
              {barbellWeightError ? (
                <Text style={styles.errorText}>{barbellWeightError}</Text>
              ) : null}
            </View>
          </View>
          ...
          <TouchableOpacity 
            style={[
              styles.signupButton, 
              { 
                backgroundColor: barbellWeightError ? theme.textMuted : theme.primary,
                opacity: barbellWeightError ? 0.6 : 1
              }
            ]}
            onPress={handleSignup}
            disabled={isLoading || !!barbellWeightError}
          >
            {isLoading ? (
